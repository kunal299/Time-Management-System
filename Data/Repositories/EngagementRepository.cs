...
namespace UCITMS.Data.Repositories
{
    public class EngagementRepository : IEngagementRepository
    {
        ...
        #region Get Project Owners

        public async Task<List<OwnerDTO>> GetProjectOwnersAsync()
        {
            var owners = new List<OwnerDTO>();
            ...
            SELECT 
                ...
                EOM.CreatedOn,
                EOM.ModifiedOn
            FROM EngagementOwnersMapping EOM
            INNER JOIN Users U ON EOM.UserID = U.UserID
        ...
        public async Task<IEnumerable<GetAllEngagementsDTO>> GetEngagementsforEmployeeAsync(int? id)
        {
            ...
            var engagements = new List<GetAllEngagementsDTO>();
                ...
                using (var connection = new SqlConnection(_connectionString))
                {
                    using (var command = new SqlCommand("dbo.GetEngagementsForEmployee_v2", connection))
                    {
                        command.CommandType = CommandType.StoredProcedure;
